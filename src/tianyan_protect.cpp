...
// 封禁设备ID
[[nodiscard]] bool TianyanProtect::BanDeviceID(const TianyanCore::BanIDPlayer& banPlayer) const {
    // 检查该设备ID是否已经存在于缓存中
...
// 解封设备ID
[[nodiscard]] bool TianyanProtect::UnbanDeviceID(const std::string& deviceId) const {
...
// 根据玩家名检查是否在黑名单中
bool TianyanProtect::isPlayerBanned(const std::string& playerName) {
...
// 根据设备ID查找被封禁的玩家信息
std::optional<TianyanCore::BanIDPlayer> TianyanProtect::getBannedPlayerByDeviceId(const std::string& deviceId) {
...
// 更新指定设备ID的玩家名称
bool TianyanProtect::updatePlayerNameForDeviceId(const std::string& deviceId, const std::string& newPlayerName) {
    try {
        // 更新缓存中的数据
        bool foundInCache = false;
...
// 计算实体密度
TianyanCore::DensityResult TianyanProtect::calculateEntityDensity(endstone::Server &server,int size) {
